(function() {
	...
	var proxyXHRForBeaconReport, originalXHRRequest;
	...
	MutationHandler.prototype.addEvent = function(resource) {
		...
		if (last_ev) {
			// If last event is submit action, returning null
			// so that multiple XHRs will be grouped into single transaction.
			if (last_ev.type === 'submit') {
				console.error('addEvent: returning from submit');
				return null;
			}
				...
				if (last_ev.nodes_to_wait === 0 || !last_ev.resource.url) {
					...
					console.error('addEvent: returning from click');
					return null;// abort
				}
				...
				// 3.2
				if (ev.type === "click") {
					console.error('addEvent: returning from XHR + Click');
					return null;
				}
				...
				if (ev.type === "xhr") {
					console.error('addEvent: returning from SPA+XHR');
					return null;
				}
			}
		}
		...
		console.error('addEvent: adding' + ev.type);
		...
		if (!MutationHandler.observer) {
			...
			if (ev.type === 'submit') {
				// if event is submit, waiting for 1.5 second
				console.error('initiating timeout');
				this.setTimeout(BOOMR.plugins.CXOptimize.groupXHRTimeout, index);
			}
	...
	MutationHandler.prototype.sendResource = function(resource, eventIndex) {
		...
		var sendResponseEnd = function(markEnd) {
			...
			if (resource.timing &&
			    ...
			    var r = BOOMR.plugins.CXOptimize.getResourceTiming(
					resource.timing.requestStart,
					resource.timing.loadEventEnd);

				 BOOMR.addVar("cxoptimize.xhr.resources", JSON.stringify(r["restime"]));
				 BOOMR.addVar("cxoptimize.xhr.navStart", r["navStart"]);
			}
			
			BOOMR.addVar("cxoptimize.details.initiator", resource.initiator)

			if (resource.transactionName)
				BOOMR.addVar("cxoptimize.xhr.transactionName", resource.transactionName);
			...
			if (BOOMR.utils.inArray(resource.initiator, BOOMR.constants.BEACON_TYPE_SPAS)) {
				...
				BOOMR.addVar("cxoptimize.spa", 
						JSON.stringify(
							{
								'initiator': resource.initiator,
								'timing': resource.timing,
								'backEndTime' : resource.timers ? resource.timers.t_resp : 'Not Available',
								'frontEndTime' : resource.timers ? resource.timers.t_page : 'Not Available',
								'totalTime' : resource.timers ? resource.timers.t_done : 'Not Available'
							}));
			}
	...
	MutationHandler.prototype.timedout = function(index) {
		...
		console.error('Firing timeout event.');

		if (ev && BOOMR.utils.inArray(ev.type, BOOMR.constants.BEACON_TYPE_SPAS.concat("xhr", "submit"))) {
			// XHRs or SPA page loads
			if (ev.type === "xhr" || ev.type === "submit") {
	...
	MutationHandler.prototype.mutation_cb = function(mutations) {
		...
		if (index < 0 || !this.pending_events[index]) {
			...
			console.error('Nothing waiting for mutations');
			return true;
		}

		evt = this.pending_events[index];
		
		console.error('mutation_cb: ' + evt.type);
	...
	function instrumentEvents() {
		...
		document.addEventListener("keydown", function(evt) {
			...
			if (evt.keyCode == 13 || evt.keyCode == 32) {
				addEventResource(evt.target, "keydown");
			}
		}, false);
		
		BOOMR.subscribe("click", function() {
			addEventResource(arguments[0], "click");
		});
	}

	function addEventResource(target, defaultInitiator) {
		if (singlePageApp) {
		...
		var resource = resource = { timing: {}, initiator: defaultInitiator };
		if (BOOMR.plugins.CXOptimize.groupXHRRequests) {
			if (target && target.type) {
					if (target.type === 'submit' || target.type === 'button')
						resource = { timing: {}, initiator: "submit", transactionName: target.value  };
					else if (target.nodeName === 'IMG' || target.nodeName === 'img' )
						resource = { timing: {}, initiator: "submit", transactionName: target.innerHTML  };
	...
	function instrumentXHR() {
		...
		BOOMR.orig_XMLHttpRequest = originalXHRRequest = BOOMR.window.XMLHttpRequest;;
	...
	BOOMR.plugins.AutoXHR = {
		...
		init: function (config) {
			...
			alwaysSendXhr = false; //&& config.AutoXHR.alwaysSendXhr;
